...
void iqs266_init() {
  driver->bsp_init();
  // initialize iqs266 states
  while (!iqs266_req_communication()) {
    iqs266_clear_reset(true);
    iqs266_read_events(false);
  }
}

bool iqs266_req_communication() {
  bool responded = false;
  uint16_t count = 0;
  driver->handshake();
  while (driver->read_rdy_pin() == 1) {
    count ++;
    driver->bsp_delay_ms(1);
    if (count % 1000 == 0)
      return responded;
    if (count % 100 == 0)
      driver->handshake();  
...
void iqs266_clear_reset(bool restart) {
  uint8_t buffer[2] = {0};
  driver->i2c_read_bytes(PROXSETTINGS_23, buffer, 2, true);
  buffer[1] |= ACK_RESET_BIT;
  driver->i2c_write_bytes(PROXSETTINGS_23, buffer, 2, restart);
}
...
void iqs266_disable_wdt(bool restart) {
  uint8_t buffer[2] = {0};
  driver->i2c_read_bytes(PROXSETTINGS_01, buffer, 2, true);
  buffer[1] |= WDT_DISABLE_BIT;
  driver->i2c_write_bytes(PROXSETTINGS_01, buffer, 2, restart);
}
...
void iqs266_auto_tune(bool restart) {
  ...
  driver->i2c_read_bytes(PROXSETTINGS_01, buffer, 1, true);
  ...
  driver->i2c_write_bytes(PROXSETTINGS_01, buffer, 1, restart);
}
...
uint8_t iqs266_read_device_info(bool restart) {
  uint8_t buffer[2];
  driver->i2c_read_bytes(DEVICE_INFO, buffer, 2, true);
  return buffer[0];
}
...
uint8_t iqs266_read_system_flags(bool restart) {
  uint8_t buffer[1];
	driver->i2c_read_bytes(SYS_FLAGS, buffer, 1, restart);
	return buffer[0];
}
...
uint8_t iqs266_read_trackpad_flags(bool restart) {
  uint8_t buffer[1]; 
  driver->i2c_read_bytes(TP_FLAGS, buffer, 1, restart);
  return buffer[0];
}
...
uint8_t iqs266_read_events(bool restart) {
  ...
  driver->i2c_read_bytes(SYS_FLAGS, buffer, 2, restart);
  return buffer[1];
}
...
uint8_t iqs266_read_approximate(bool restart) {
  uint8_t buffer[1];
  driver->i2c_read_bytes(CHANNEL_BYTES, buffer, 1, restart);
  return buffer[0];
}
...
uint8_t iqs266_read_touch(bool restart) {
  uint8_t buffer[2];
  driver->i2c_read_bytes(CHANNEL_BYTES, buffer, 2, restart);
...
uint8_t iqs266_read_x(bool restart) {
  ...
  driver->i2c_read_bytes(COORDINATES, buffer, 1, restart);
...
uint8_t iqs266_read_y(bool restart) {
  ...
  driver->i2c_read_bytes(COORDINATES, buffer, 2, restart);
...
uint16_t iqs266_read_channel_counts(channel_t channel, bool restart) {
  ...
  driver->i2c_read_bytes(start_address, buffer, 2, restart);
...
uint16_t iqs266_read_channel_lta(channel_t channel, bool restart) {
  ...
  // Read the bytes with the readRandomBytes method
  driver->i2c_read_bytes(read_address, buffer, 2, restart);
  // Construct the 16bit return value.
  lta_low = buffer[0];
  lta_high = buffer[1];
...
void iqs266_set_halt_timeout(uint8_t timeout, bool restart) {
  ...
  driver->i2c_write_bytes(TIMEOUT_PERIODS, &timeout, 1, restart);
}
...
void iqs266_set_np_segment_rate(uint8_t value, bool restart) {
  uint8_t buffer[1];
	...
	driver->i2c_read_bytes(PROXSETTINGS_23, buffer, 1, true);
	...
	driver->i2c_write_bytes(PROXSETTINGS_23, buffer, 1, restart);
}
...
void iqs266_set_report_rate_lp(uint8_t value, bool restart) {
  uint8_t buffer[2];
	...
	driver->i2c_read_bytes(REPORT_RATES, buffer, 1,  true);
	// Set the report rate for the low power mode.
	buffer[1] = value;
	// Now write all the bytes back to the REPORT_RATES address.
	driver->i2c_write_bytes(REPORT_RATES, buffer, 2, restart);
}
...
void iqs266_set_report_rate_nm(uint8_t value, bool restart) {
	...
	driver->i2c_write_bytes(REPORT_RATES, &value, 1, restart);
}
...
void iqs266_set_touch_ati_target(uint8_t value, bool restart) {
  // No need to read any information since we are just over-writing a value into the first byte at the address.
	...
	driver->i2c_write_bytes(ATI_TARGETS, &value, 1, restart);
}
...
void iqs266_set_touch_base_value(uint8_t value, bool restart) {
	...
	driver->i2c_read_bytes(CHANNEL_SETTINGS, buffer, 1, true);
	...
	driver->i2c_write_bytes(CHANNEL_SETTINGS, buffer, 1, restart);
}
...
void iqs266_set_touch_sensitivity(channel_t channel, uint8_t sensitivity, bool restart) {
  ...
  driver->i2c_read_bytes(address, buffer, 2, true);
  switch (channel)
  {
  ...
  if (channel == ALL)
    driver->i2c_write_bytes(address, buffer, 8, restart);
  else
    driver->i2c_write_bytes(address, buffer, 2, restart);
...
void iqs266_set_proximate_sensitivity(uint8_t sensitivity, bool restart) {
  ...
  driver->i2c_write_bytes(THRESHOLDS_CH0, buffer, 1, restart);
}
...
void iqs266_set_tap_timeout(uint8_t timeout, bool restart) {
  ...
  driver->i2c_write_bytes(TAP_SETTINGS, buffer, 1, restart);
}
...
void iqs266_set_swipe_timeout(uint8_t timeout, bool restart) {
  ...
  buffer[0] = timeout;
  // Write the time limit to the correct address.
  driver->i2c_write_bytes(SWIPE_SETTINGS, buffer, 1, restart);
}
...
void iqs266_set_swipe_threshold(uint8_t threshold, bool restart) {
  ...
  driver->i2c_read_bytes(SWIPE_SETTINGS, buffer, 1, true);
  // Store the the threshold to the second byte which will be transferred.
  buffer[1] = threshold;
  // Write the Swipe settings bytes to the correct address.
  driver->i2c_write_bytes(SWIPE_SETTINGS, buffer, 2, restart);
}
...
static void _set_event_mask_bits(uint8_t bit_mask, bool restart) {
  ...
  driver->i2c_write_bytes(EVENT_MASK, buffer, 1, restart);
}
...
void iqs266_disable_channel(channel_t channel, bool restart) {
  ...
  buffer[1] &= ~(1<<channel);
  // Write the new value to the ATCTIVE_CHANNELS address
  driver->i2c_write_bytes(CHANNEL_SETTINGS, buffer, 2, true);
}
